...
namespace UnityEngine.UI.Extensions
{
	...
	public class AccordionElement : Toggle
	{
        ...
        [SerializeField] private float OnHeight = 100.0f;
		...
		public void OnValueChanged(bool state)
		{
			...
			if (transition == Accordion.Transition.Instant)
			{
				if (state)
				{
                    this.m_LayoutElement.preferredHeight = OnHeight;
				}
				else
				{
			...
			else if (transition == Accordion.Transition.Tween)
			{
				if (state)
				{
                    this.StartTween(this.m_MinHeight, OnHeight);
				}
				else
				{
